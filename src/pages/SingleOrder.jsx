			...
			try {
				...
				const supPrice = result.orders.products.reduce((acc, product) => {
					if (product.supplementryProducts) {
						return (
							acc +
							product.supplementryProducts.reduce((sum, item) => sum + item.price, 0)
						);
					}
					return acc;
				}, 0);
					...
					<div className="flex items-center justify-between">
						<p
							className={
								order.status === "PENDING"
									...
									? "text-medium text-success"
									: order.status === "CANCELLED"
									? "text-medium text-danger"
						...
						<div>
							<p className="text-medium">
								<span className="capitalize font-bold">Subtotal:</span> &#8358;{" "}
							...
							<p className="text-medium">
								<span className="capitalize font-bold">VAT:</span> &#8358;{" "}
							...
							<p className="text-medium">
								<span className="capitalize font-bold">Charges:</span> &#8358;{" "}
							...
							<p className="text-medium">
								...
								{parseFloat(order.netAmount).toLocaleString("en-US", {
									minimumFractionDigits: 2,
									maximumFractionDigits: 2,
								})}
							</p>
									...
									<div className="flex space-x-2 items-center">
										{item.variation !== null && (
											<div className="flex space-x-2 items-center">
												...
												item.variation.type === "others" ? (
													<p className="uppercase">{item.variation.variant}</p>
												) : (
													<div
														className="w-4 h-4 rounded-full"
														style={{ backgroundColor: item.variation.variant }}
													/>
												)}
											</div>
										)}
								...
								{item.supplementryProducts && item.supplementryProducts.length > 0 && (
									<div className="mt-4">
										<p className="font-bold text-small capitalize">
											Supplementary Product Informations
										</p>
											...
											{item.supplementryProducts.map((sup) => (
												<div
													...
													<div className="flex justify-between">
														...
														<p className="font-bold">
															&#8358;
															{sup.product.price.toLocaleString("en-US", {
																minimumFractionDigits: 2,
																maximumFractionDigits: 2,
															})}
														</p>
													...
													<div>
														<p>
															Total Cost: &#8358;{" "}
															{sup.price.toLocaleString("en-US", {
																minimumFractionDigits: 2,
																maximumFractionDigits: 2,
															})}
														</p>
