...
import { Button, Image, Radio, RadioGroup, Skeleton, useDisclosure } from "@nextui-org/react";
...
const Checkout = () =>
{
  ...
  const [ isDisabled, setisDisabled ] = useState((!user.address && !user.address?.location) || 
  ...
  {
    if ( user.balance < cartTotal + ( cartItems.length * 200 ) ) {
      setisDisabled( true );
      openToast( "Insufficient balance", "error" )
      return;
    }
    const data = {
      userId: user.id,
      ...
      orderProduct: cartItems.map(item=>({productId: item.productId,price:item.price,quantity:item.quantity,variation:item.variation,supplementryProducts:JSON.stringify(item.suplementryProducts)}))
    }
    try {
  ...
  return (
    <div className="h-full">
        ...
        <div className="h-full">
          ...
          <section className="h-full flex flex-col lg:grid lg:grid-cols-5 lg:gap-4">
            ...
            <div className="sticky mx-auto w-full flex justify-between items-center border-t-1 p-2 lg:flex-col border lg:col-span-2 h-fit lg:h-64 lg:items-start mt-2 lg:mt-0 rounded-s-md">
              ...
              <Button radius="full" color="primary" className="flex space-x-2 w-1/2 lg:w-full" onClick={()=>setDispatchActive(prev=>!prev)} isLoading={loading} isDisabled={isDisabled}>
                <FaCartShopping />
      ...
      { isDispatchActive && (
        <DeliveryAddress isLoading={loading} isDisabled={isDisabled} checkOut={checkOut} setDispatchActive={setDispatchActive} setDispatchCenter={setDispatchCenter}/>
      )}
    </div>
  )
}

const DeliveryAddress = ({isLoading,isDisabled,checkOut,setDispatchCenter,setDispatchActive}) =>
{
  const { fetchData,loading } = useAxiosFetch();
  ...
  return (
    <div className="h-full flex gap-4 flex-col">
      <div>
        <BsArrowLeft size={20} onClick={()=>setDispatchActive(prev=>!prev)}/>
      </div>
      <div className="flex-1 overflow-y-auto">
        ...
        { !loading && locations.length > 0 && (
          <RadioGroup label="Select dispatch location" onChange={(e)=>setDispatchCenter(e.target.value)}>
          { locations.map( location => (
            <Radio key={ location.id } value={`${location.address}, ${location.city}, ${location.state}, ${location.country}`}>{ `${location.address}, ${location.city}, ${location.state}, ${location.country}` }</Radio>
          ))}
        </RadioGroup>
        )}
      </div>
      <Button radius="full" color="primary" className="flex space-x-2 w-1/2 lg:w-full" onClick={checkOut} isLoading={isLoading} isDisabled={isDisabled}>
        <FaCartShopping />
        <p>Checkout</p>
      </Button>
    </div>
