			...
			{cartItems.length > 0 && !isLoading && (
				<section className="h-full flex flex-col">
					...
					<div className="flex-1">
						{cartItems.map((cart, index) => (
							<div
								...
								className="grid grid-cols-1 mb-2 border rounded-lg p-2">
									...
									<div className="flex-1">
										<div className="flex flex-between mb-4">
												...
												<p className="text-primary font-bold text-sall">
													{cart.variation && (
														<span
															className={
																cart.price
																	? "line-through italic me-1 text-tiny font-thin text-danger"
																	: ""
															}>
															&#8358;
															{cart.product.variation.find(
																(variation) => variation.variant === cart.variation.variant
															)?.price || 0}
														</span>
													)}
													...
													<span
														className={
															!cart.product.discount_amount ? "hidden" : "inline-block"
														}>
														&#8358;
														{cart.price.toLocaleString("en-US", {
															minimumFractionDigits: 2,
											...
											<div className="">
												<div
													role="button"
													className="flex justify-center items-center border border-primary bg-white rounded-full p-2 w-8 h-8 md:p-2">
													<BsHeart
														size={20}
										...
										<div className="flex justify-evenly items-center font-bold">
											<div className="flex space-x-2 md:space-x-4 items-center lg:w-3/4">
												<Button
													...
													onClick={() =>
														handleDecrement({
															id: cart.id ? cart.id : cart.productId,
															variant: cart.variation ? cart.variation.variant : null,
														})
													}>
													<BsDash size={20} />
												</Button>
											...
											<Button
												className="flex space-x-4 lg:w-1/4"
												size="sm"
												color="danger"
												variant="flat"
												onClick={() =>
									...
									<div className="mt-1">
										...
										{cart.suplementryProducts.map((product, supIndex) => (
											<div
												...
												<div>
													<p className="text-tiny font-bold ms-2 me-4">
												...
												<button
													className="px-4 py-1 rounded-lg border-0 bg-neutral-200 hover:bg-neutral-300"
													onClick={() => decreaseSup(index, supIndex)}>
													{" "}
													<BsDash />
												</button>
												...
												<button
													className="px-4 py-1 rounded-lg border-0 bg-neutral-200 hover:bg-neutral-300"
													onClick={() => increaseSup(index, supIndex)}>
													{" "}
													<BsPlus />
												</button>
											</div>
					...
					<div className="sticky mx-auto w-full flex justify-between items-center border-t-1 p-2">
						<div>
							<div>
								<p className="text-tiny md:text-small lg:text-medium font-bold">
									Subtotal: ₦
						...
						<Button
							radius="full"
							color="primary"
							className="flex space-x-2 w-1/2"
							onClick={checkoutBtn}>
							<FaCartShopping />
							<p>Checkout</p>
						</Button>
